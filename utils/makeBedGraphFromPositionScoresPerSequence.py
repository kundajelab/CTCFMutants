...
def parseArgument():
	...
	parser.add_argument("--outputFileNamePrefix", required=True,\
			help='Prefix of file name where regions, their top positions, and the scores of those positions will be recorded')
...
def makeBedGraphFromPositionScoresPerSequence(options):
	# Get the positions with the largest scores and make a separate output file for each sequence
	...
	for line in regionsFile:
		...
		outputFileName = options.outputFileNamePrefix + "_" + str(lineNum) + ".bedgraph"
		outputFile = open(outputFileName, 'w+')
		# Write the header to the output file
		#outputFile.write("track type=bedGraph name=\"" + options.trackName + "_" + str(lineNum) + "\"\n")
