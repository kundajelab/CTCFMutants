...
def parseArgument():
	...
	parser.add_argument("--codePath", required=False, default="/srv/scratch/imk1/TFBindingPredictionProject/src/", \
	...
	Y = None
		...
		if ((not options.peakInfoFileName.endswith("fa")) and (not options.peakInfoFileName.endswith("fasta"))) and \
			...
			X, Y = makePositiveSequenceInputArraysFromFasta(options.peakInfoFileName, dataShape=(1,4,options.sequenceLength))
                ...
                summitPlusMinus =\
			makeSummitPlusMinus(options.peakInfoFileName, createOptimalBed=False, dataShape=(1,4,options.sequenceLength), summitCol=1, \
				startCol=None)
                ...
                X, Y, _, _, _, indices =\
                        makeSequenceInputArraysFromDifferentialPeaks(options.DESeq2OutputFileName, options.genomeFileName, options.peakInfoFileName, \
                                (1,4,options.sequenceLength), createOptimalBed=False, backgroundSummitPresent=False, backgroundSummitOnly=True, \
				createModelDir=False, chroms=options.chroms, bigWigFileNames=[], multiMode=False, streamData=False, dataFileName="", \
				RC=False)
        data = None
	if options.negSet:
		# Run TF-Modisco on the negatives
		data = X[Y == 0,:,:,:]
	else:
		# Run TF-Modisco on the positives
		data = X[Y == 1,:,:,:]
